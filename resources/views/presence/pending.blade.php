                ...
                <table id="zero-config" class="table dt-table-hover" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$val["status"] == 3 ? "Sakit": "Izin"}}</td>
                            <td>{{$val["time_in"]}}</td>
                            <td>{{$val["time_out"]}}</td>
                            <td class="d-flex">
                                <a href="{{url('presence/'.$key)}}" class="btn btn-outline-dark"><i class="fas fa-eye"></i></a>
                                <form action="{{ route("presence.approve", $key) }}" method="POST">
                                    @csrf
                                    <button class="btn btn-outline-success ml-2"><i class="fas fa-check"></i></button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
